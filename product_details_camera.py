...
csv_filename = 'product_details_camera.csv'
csv_bought_together = 'items_bought_together_camera.csv'
csv_customer_reviews = 'links_to_customer_reviews_camera.csv'
txt_customer_reviews = 'links_to_customer_reviews_camera.txt'

df = pd.read_json('camera_product_output.jsonl')
...
df_camera_customer_reviews_link = df.link_to_all_reviews
df_camera_customer_reviews_link = df_camera_customer_reviews_link.dropna()
df_camera_customer_reviews_link.reset_index(drop=True, inplace = True)

df_camera_bought_together = df[['ASIN', 'name', 'freq_bought', 'freq_bought_link']]

df.to_csv(csv_filename)
df_camera_customer_reviews_link.to_csv(csv_customer_reviews) # actually not necessary anymore since there's being a txt-file created
df_camera_bought_together.to_csv(csv_bought_together)

f = open(txt_customer_reviews, 'w+')
for i in range(len(df_camera_customer_reviews_link)):
    f.write(df_camera_customer_reviews_link[i])
    f.write('\n')
f.close()
