...
csv_filename = 'product_details_headphone.csv'
csv_bought_together = 'items_bought_together_headphone.csv'
csv_customer_reviews = 'links_to_customer_reviews_headphone.csv'
txt_customer_reviews = 'links_to_customer_reviews_headphone.txt'

df = pd.read_json('headphone_product_output.jsonl')
...
df_headphone_customer_reviews_link.reset_index(drop=True, inplace = True)

df_headphone_bought_together = df[['ASIN', 'name', 'freq_bought', 'freq_bought_link']]

df.to_csv(csv_filename)
df_headphone_customer_reviews_link.to_csv(csv_customer_reviews) # actually not necessary anymore since there's being a txt-file created
df_headphone_bought_together.to_csv(csv_bought_together)

f = open(txt_customer_reviews, 'w+')
for i in range(len(df_headphone_customer_reviews_link)):
    f.write(df_headphone_customer_reviews_link[i])
    f.write('\n')
f.close()
