...
csv_filename = 'product_details_mouse.csv'
csv_bought_together = 'items_bought_together_mouse.csv'
csv_customer_reviews = 'links_to_customer_reviews_mouse.csv'
txt_customer_reviews = 'links_to_customer_reviews_mouse.txt'

df = pd.read_json('mouse_product_output.jsonl')
...
df_mouse_customer_reviews_link = df.link_to_all_reviews
df_mouse_customer_reviews_link = df_mouse_customer_reviews_link.dropna()
df_mouse_customer_reviews_link.reset_index(drop=True, inplace = True)

df_mouse_bought_together = df[['ASIN', 'name', 'freq_bought', 'freq_bought_link']]

df.to_csv(csv_filename)
df_mouse_customer_reviews_link.to_csv(csv_customer_reviews) # actually not necessary anymore since there's being a txt-file created
df_mouse_bought_together.to_csv(csv_bought_together)

f = open(txt_customer_reviews, 'w+')
for i in range(len(df_mouse_customer_reviews_link)):
    f.write(df_mouse_customer_reviews_link[i])
    f.write('\n')
f.close()
